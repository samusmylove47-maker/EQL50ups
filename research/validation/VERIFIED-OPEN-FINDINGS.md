...
> **Closed** in the screen, not the pipeline — and the caveat you raised is the reason. The pipeline cannot fold `sessions` either: upstream a session is `{date, zone, difficulty}`, which cannot tell two evenings from one recorded twice, and the field's meaning is not consistent across rows (over 614 upstream rows, `sessions.length === seen` for 381 and differs for 233). Folding there would write an unmeasured number into a published shape. At read time the uncertainty can be stated instead: a folded row reports the larger count and the card says "over 4 sessions or more". Rows for one mob in **different zones** are deliberately not folded — unioning their zones would push the count out of `zoneTallies`' placed bucket. Result: the same 17 items corrected, 0 other figures moved, asserted over every shipped item. See the CLOSED note under F04 in `AUDIT-UPGRADES-SURFACE.md`.
...
> **Closed**, as described, with `take()`'s callback returning a value to maximise rather than a boolean, and the Lore claim moved to the winner alone. Two things this record's numbers did not survive contact with. (1) My own A/B over 220 shipped-payload configurations gives **10 changed (4.5%)**, not 264 of 620 — different sweeps, so neither confirms the other, and I have not reprinted yours. (2) The cascade is real and larger than "Primary only": over all 4,776 rows, **20 differ** — PRIMARY 10, ANY_1 6, SECONDARY 4 — because `Dagas` is LORE and lists both hands, and `Baton of the Sky` is LORE and PRIMARY-only, so freeing it hands ANY_1 the same 57.5 EP. Every non-Primary change is the single-copy hand-out reallocating correctly. See the CLOSED note under F12 in `AUDIT-UPGRADES-SURFACE.md`.
